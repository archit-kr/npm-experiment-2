...
function renderGraph(root, data) {
  const layout = {
    title: `Commits to the storybookjs/storybook repo (${new Date(data[0].week * 1000).toLocaleDateString()} - ${new Date(data[data.length - 1].week * 1000).toLocaleDateString()})`,
    xaxis: {
      title: "Week",
      showgrid: true,
      zeroline: false,
...
function renderGrid(root, data) {
  const layout = {
    title: `Commits to the storybookjs/storybook repo, by day`,
    height: 350,
  };
